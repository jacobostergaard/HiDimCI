...
S_moments <- function(X){
  ...
  S11 = S(R1,R1)
  OLS = S01%*%solve(S11)
  out = list(S00=S00, S01=S01, S10=S10, S11=S11, OLS=OLS)
  return(out)
}
...
project_lift <- function(target, rank, project=sym, maxtol=1e-5, maxiter=100, reverse=TRUE, verbose=FALSE){
  M0  = target
  tol = 1
  i   = -1
  if(length(rank)>1){
    M   = array(dim=c(nrow(M0),ncol(M0),length(rank)))
    tmp = numeric(length(rank))
    for(j in 1:length(rank)){
      M[,,j] = project_lift(target, rank[j], project, maxtol, maxiter, reverse, verbose=FALSE)
      tmp[j] = frobenius(M[,,j]-target)
    }
    out = M[,,which(tmp==min(tmp))]
  } else{
    while(tol>maxtol & i<maxiter){
      i     = i+1
      if(reverse){
        # Reverse operations (project-and-lift)
        M.tmp = project(M0)
        out   = low_rank(M.tmp,rank)
      } else{
        # Original steps of lift-and-project
        M.tmp = low_rank(M0,rank)
        out   = project(M.tmp)
      }
      tol   = frobenius(out-M0)
      M0    = out
    }
    if(verbose){
